...
func TestKernelInfo(t *testing.T) {
	...
	for i, tt := range table {
		kernel_info, err = NewKernelInfo(tt.kernel)
		if err != nil {
			t.Fatalf("NewKernelInfo failed (test #%d): %s", i, err.Error())
		}
...
func TestKernelInfoBuiltIn(t *testing.T) {
	...
	kernel_info, err := NewKernelInfoBuiltIn(KERNEL_RING, "2,1")
	if err != nil {
		t.Fatalf("NewKernelInfoBuiltIn failed: %s", err.Error())
	}
